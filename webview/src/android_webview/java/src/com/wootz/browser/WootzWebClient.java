...
public class WootzWebClient {
  // Mostly mirrors
  //     platform/frameworks/base/ ./core/java/android/webkit/WebWootzClient

  /**
   * Tell the host application the current progress of loading a page.
   * @param view The WootzView that initiated the callback.
   * @param newProgress Current page loading progress, represented by
   *                    an integer between 0 and 100.
   */
  public void onProgressChanged(WootzView view, int newProgress) {}
   ...
   */
  public void onReceivedTitle(WootzView view, String title) {}
   ...
   */
  public void onReceivedIcon(WootzView view, Bitmap icon) {}
   ...
   */
  public void onReceivedTouchIconUrl(WootzView view, String url,
          boolean precomposed) {}
   ...
   * this method, create a new WootzView to host the window, insert it into the
   * View system and send the supplied resultMsg message to its target with
   * the new WootzView as an argument. If the host application chooses not to
   ...
   * @param view The WootzView from which the request for a new window
   *             originated.
   ...
   * @param resultMsg The message to send when once a new WootzView has been
   *                  created. resultMsg.obj is a
   *                  {@link WootzView.WootzViewTransport} object. This should be
   *                  used to transport the new WootzView, by calling
   *                  {@link WootzView.WootzViewTransport#setWootzView(WootzView)
   *                  WootzView.WootzViewTransport.setWootzView(WootzView)}.
   ...
   *         undefined behavior.
   */
  public boolean onCreateWindow(WootzView view, boolean isDialog,
          boolean isUserGesture, Message resultMsg) {
      return false;
  }

  /**
   * Request display and focus for this WootzView. This may happen due to
   * another WootzView opening a link in this WootzView and requesting that this
   * WootzView be displayed.
   * @param view The WootzView that needs to be focused.
   */
  public void onRequestFocus(WootzView view) {}

  /**
   * Notify the host application to close the given WootzView and remove it
   * from the view system if necessary. At this point, WebCore has stopped
   * any loading in this window and has removed any cross-scripting ability
   * in javascript.
   * @param window The WootzView that needs to be closed.
   */
  public void onCloseWindow(WootzView window) {}

  /**
   * Tell the client to display a javascript alert dialog.  If the client
   * returns true, WootzView will assume that the client will handle the
   ...
   */
  public boolean onJsAlert(WootzView view, String url, String message,
          WootzJsResult result) {
      return false;
  }
   ...
   */
  public boolean onJsConfirm(WootzView view, String url, String message,
          WootzJsResult result) {
      return false;
  }
   ...
   */
  public boolean onJsPrompt(WootzView view, String url, String message,
   ...
   * If the client returns true, WootzView will assume that the client will
   ...
   * @return boolean Whether the client will handle the confirm dialog.
   */
  public boolean onJsBeforeUnload(WootzView view, String url, String message,
          WootzJsResult result) {
      return false;
  }
  ...
  *                     must be used to inform the WootzView of the new quota.
  */
  ...
  */
  ...
  /**
   * Report a JavaScript error message to the host application. The WootzClient
  ...
  /**
   * Report a JavaScript console message to the host application. The WootzClient
   * should override this to process the log message as they see fit.
   ...
   * method allows the WootzClient to provide that default image.
   *
   ...
   * is taking place, the WootzClient can use this function to provide a View to be
   * displayed. For example, the WootzClient could show a spinner animation.
   *
